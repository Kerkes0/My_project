...
void wifi_uart_write_frame(unsigned char fr_type, unsigned char fr_ver, unsigned short len)
{
    ...
    wifi_uart_tx_buf[len - 1] = check_sum;
		...
		WIFI_U2_saveData((char*)wifi_uart_tx_buf, len);//20220316+更改wifi串口的发送方式
    //wifi_uart_write_data((unsigned char *)wifi_uart_tx_buf, len);//20220316-更改wifi串口的发送方式
}
...
void data_handle(unsigned short offset)
{
    ...
    switch(cmd_type)
    {
        ...
        case DATA_QUERT_CMD:                                    //命令下发 /////////////20211224对接收app的数据进行处理
