...
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
#define PRODUCT_KEY "i0elff0w1pehctax"    //开发平台创建产品后生成的16位字符产品唯一标识
...
#define WIFI_UART_RECV_BUF_LMT          20              //串口数据接收缓存区大小,如MCU的RAM不够,可缩小//yxl-wifi
#define WIFI_DATA_PROCESS_LMT           30              //串口数据处理缓存区大小,根据用户DP数据大小量定,必须大于24//yxl-wifi
#else
...
#define WIFIR_UART_SEND_BUF_LMT         60              //根据用户DP数据大小量定,必须大于48//yxl-wifi
/******************************************************************************
                        4:定义模块工作方式
模块自处理:
...
//当前功率(只上报)
//备注:
#define DPID_CUR_POWER 5
//流量设定最大值(只上报)
//备注:适用四种机型
#define DPID_FLOW_RATE_MAX 101
//实时运行速率(只上报)
//备注:
#define DPID_SPEED_REAL 102
//模式(可下发可上报)
//备注:
#define DPID_MODE 103
//反冲洗设置时间(可下发可上报)
//备注:
#define DPID_BACKWASH_TIME_SET 104
//开关机(可下发可上报)
//备注:
#define DPID_SWTICH 105
//流量设定值(可下发可上报)
//备注:适用四种机型
#define DPID_FLOW_RATE_SET 106
//流量设定最小值(只上报)
//备注:适用四种机型
#define DPID_FLOW_RATE_MIN 107
//反冲洗倒计时(只上报)
//备注:
#define DPID_BACKWASH_COUNTDOWN 108
//耗电量(只上报)
//备注:
#define DPID_POWER_CONSUMPTION 109
//单位切换(可下发可上报)
//备注:
#define DPID_UINT_SWITCH 110
//速率设定值(可下发可上报)
//备注:
#define DPID_SPEED_SET 111
//实时水流量(只上报)
//备注:适用四种流量
#define DPID_WATER_REALFLOW 112
//流量步进值(只上报)
//备注:适用四种机型
#define DPID_FLOW_STEP 113

#define DPID_FLOW_PRESS_WARN 114  
