...
#include "User.h"//yxl  包含头文件//yxl-wifi
...
extern uchar all_updata_step;//发送pro_all_updata();数据中数据的第几步骤

void WIFI_U2_cycleInit(void);//环形阵列的初始化
void WIFI_U2_SendData(void);//将环形阵列的数据发给wifi模块
char WIFI_U2_saveData(char* sdata ,unsigned short len);//将要发送的数据塞入（缓存）环形阵列中
void getoveru2data(void);//获取溢出次数

void pro_wifi_handle(void);
void pro_wifi_updata(void);
void pro_all_updata(void);//20220314+
