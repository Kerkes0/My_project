...
{
    // #error "请将MCU串口发送函数填入该函数,并删除该行"//yxl-wifi
...
{
    all_updata_step = 1; // 20220314+发送all数据的第几步骤，为1时启用Wifi.c文件中的void pro_all_updata()函数

    //  #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"//yxl-wif
    // /*
    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    //    mcu_dp_fault_update(DPID_FAULT,当前故障); //故障型数据上报;
    //    mcu_dp_value_update(DPID_CUR_POWER,当前当前功率); //VALUE型数据上报;
    //    mcu_dp_value_update(DPID_FLOW_RATE_MAX,当前流量设定最大值); //VALUE型数据上报;
    //    mcu_dp_value_update(DPID_SPEED_REAL,当前实时运行速率); //VALUE型数据上报;
    //    mcu_dp_enum_update(DPID_MODE,当前模式); //枚举型数据上报;
    //    mcu_dp_value_update(DPID_BACKWASH_TIME_SET,当前反冲洗设置时间); //VALUE型数据上报;
    //    mcu_dp_bool_update(DPID_SWTICH,当前开关机); //BOOL型数据上报;
    //    mcu_dp_value_update(DPID_FLOW_RATE_SET,当前流量设定值); //VALUE型数据上报;
    //    mcu_dp_value_update(DPID_FLOW_RATE_MIN,当前流量设定最小值); //VALUE型数据上报;
    //    mcu_dp_value_update(DPID_BACKWASH_COUNTDOWN,当前反冲洗倒计时); //VALUE型数据上报;
    //    mcu_dp_value_update(DPID_POWER_CONSUMPTION,当前耗电量); //VALUE型数据上报;
    //    mcu_dp_enum_update(DPID_UINT_SWITCH,当前单位切换); //枚举型数据上报;
    //    mcu_dp_value_update(DPID_SPEED_SET,当前速率设定值); //VALUE型数据上报;
    //    mcu_dp_value_update(DPID_WATER_REALFLOW,当前实时水流量); //VALUE型数据上报;
    //    mcu_dp_value_update(DPID_FLOW_STEP,当前流量步进值); //VALUE型数据上报;
    //	 mcu_dp_bool_update(DPID_FLOW_PRESS_WARN,压力告警); ////BOOL型数据上报;

    // */

    // 20220314-由于发送的缓存限制，故采取发布发送的方法，参考Wifi.c文件中void pro_all_updata(void)函数。
...
static unsigned char dp_download_mode_handle(const unsigned char value[], unsigned short length)
{
    ...
    case 0: //接收到流量模式指令
        ...
        {
            fg_selfimbi_ok = 0;   //退出超频自吸
            //fg_rest_con_wifi = 1; // 20220105
            fg_rest_con_ok = 1;u8WashState = WashState_SelfStudy; //退出自吸,进入自学习;   //进入自学习
        }
        //超频自吸（已经自学习时）
    ...
    case 2: //接收到反冲洗模式指令
        //自学习 或 开机上水 或 超频自吸时
        if ((fg_rest_con_ok == 1) || (fg_auto_upwater == 1) || (fg_selfimbi_ok == 1)) // yxl-xue  首次不能低于180  自学习期间不可操作
        {
        }
        else
        {
...
/*****************************************************************************
函数名称 : dp_download_backwash_time_set_handle
功能描述 : 针对DPID_BACKWASH_TIME_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
备注     ：反冲洗时间，自学习和超频自吸、开机上水
*****************************************************************************/
static unsigned char dp_download_backwash_time_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    ...
    else if ((fg_rest_con_ok == 1) && (fg_selfimbi_ok == 0) && (fg_auto_upwater == 0)) //单纯自学习
    {
        //            WashTmSet = backwash_time_set;
        //            if (WashTmSet <= 180)
        //                WashTmSet = 180;
    }
    else if ((fg_selfimbi_ok == 1)&& (fg_auto_upwater == 0)) //超频自吸时
    {
        WashTmSet = backwash_time_set;
        if (WashTmSet <= 600)
            WashTmSet = 600;
    }
    else if (fg_auto_upwater == 1) //上水时
    {
    }
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_swtich_handle(const unsigned char value[], unsigned short length)
{
    ...
    if (swtich == 0)
    {
        ...
        {
            gu8KeyFlg = bPower;
            if (gu8SetMode == SPEED) // yxl-5  关机带记忆
            {
...
/*****************************************************************************
函数名称 : dp_download_flow_rate_set_handle
功能描述 : 针对DPID_FLOW_RATE_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
static unsigned char dp_download_flow_rate_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    ...
    {			
			gu8SetMode = CONTROL; //定时直接转为流量			
    }
		
    if (fg_selfimbi_ok == 1) // 20211230修复了超频自吸时显示屏接收到app的流量模式指令后不会自学习的问题
    {
        fg_rest_con_ok = 1;u8WashState = WashState_SelfStudy; //退出自吸,进入自学习;
    }
...
/*****************************************************************************
函数名称 : dp_download_uint_switch_handle
功能描述 : 针对DPID_UINT_SWITCH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
/*****************************************************************************
函数名称 : dp_download_speed_set_handle
功能描述 : 针对DPID_SPEED_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
static unsigned char dp_download_speed_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    ...
    if (fg_selfimbi_ok == 1) // 20211230修复了超频自吸时显示屏接收到app的转速模式指令后不会自学习的问题
    {
        fg_rest_con_ok = 1;u8WashState = WashState_SelfStudy; //退出自吸,进入自学习;
    }
    //处理完DP数据后应有反馈
...
{
    //#error "请自行实现wifi功能测试成功/失败代码,完成后请删除该行"//yxl-wifi
    if (result == 0)
    {
        //测试失败
        if (rssi == 0x00)
